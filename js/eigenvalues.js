...
function replot(eigs, lr, beta, blob) {
  ...
  */
  ...
  // update text
  $('.p_lr').text('lr=' + lr.toFixed(2));
  if(beta>=0){
    $('.p_sign_beta').text('positive');
  } else {
    $('.p_sign_beta').text('negative');
  }
  $('.p_beta').text('beta='+beta.toFixed(2));
  if(radius_shifted_eigs >= 1.) {
    $('.p_momentum_inside').text('outside').css('color','red');
    $('.p_momentum_converges').text('divergence').css('color','red');
  } else {
    $('.p_momentum_inside').text('inside').css('color','green');
    $('.p_momentum_converges').text('convergence').css('color','green');
  }
  if(radius_eigs >= 1.) {
    $('.p_sgd_inside').text('outside').css('color','red');
    $('.p_sgd_converges').text('divergence').css('color','red');
  } else {
    $('.p_sgd_inside').text('inside').css('color','green');
    $('.p_sgd_converges').text('convergence').css('color','green');
  }
}
...
function initialize_touchpad(class_name, pixels,
  ...
  info_box = create_unique(tp.touchpad, 'text', 'info', ['box'])
  .attr("x", 20)
  .attr("y", tp.pixels/2)
  .attr('fill', 'blue')
  .text('Move mouse over this area');
...
$(document).ready(function(){
  tp = initialize_touchpad('touchpad', 300, 0, 2);

  blob = initialize_chart('chart', 300, 2);
